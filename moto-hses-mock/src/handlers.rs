...
impl CommandHandler for StatusHandler {
    fn handle(&self, _message: &proto::HsesRequestMessage, state: &mut MockState) -> Result<Vec<u8>, proto::ProtocolError> {
...
impl CommandHandler for AxisNameHandler {
    fn handle(&self, _message: &proto::HsesRequestMessage, _state: &mut MockState) -> Result<Vec<u8>, proto::ProtocolError> {
        let mut data = vec![0u8; 56]; // 7 axes * 8 bytes each
        
        // Set default axis names
        let axis_names = ["1st_axis", "2nd_axis", "3rd_axis", "4th_axis", "5th_axis", "6th_axis", "7th_axis"];
...
impl CommandHandler for FileControlHandler {
    fn handle(&self, message: &proto::HsesRequestMessage, state: &mut MockState) -> Result<Vec<u8>, proto::ProtocolError> {
        ...
        match service {
            ...
            0x32 => { // Get file list (Python client uses this)
                // Return a simple file list
                let file_list = "TEST.JOB\0";
                Ok(file_list.as_bytes().to_vec())
            }
...
impl CommandHandlerRegistry {
    pub fn new() -> Self {
        ...
        handlers.insert(0x00, Arc::new(FileControlHandler) as Arc<dyn CommandHandler + Send + Sync>);
        ...
        handlers.insert(0x73, Arc::new(ExecutingJobInfoHandler) as Arc<dyn CommandHandler + Send + Sync>);
        ...
        handlers.insert(0x76, Arc::new(PositionErrorHandler) as Arc<dyn CommandHandler + Send + Sync>);
        ...
        handlers.insert(0x7f, Arc::new(PositionVarHandler) as Arc<dyn CommandHandler + Send + Sync>);
        handlers.insert(0x80, Arc::new(BasePositionVarHandler) as Arc<dyn CommandHandler + Send + Sync>);
        handlers.insert(0x81, Arc::new(ExternalAxisVarHandler) as Arc<dyn CommandHandler + Send + Sync>);
        ...
        handlers.insert(0x85, Arc::new(TextDisplayHandler) as Arc<dyn CommandHandler + Send + Sync>);
        ...
        handlers.insert(0x88, Arc::new(ManagementTimeHandler) as Arc<dyn CommandHandler + Send + Sync>);
        ...
        handlers.insert(0x84, Arc::new(SelectCycleHandler) as Arc<dyn CommandHandler + Send + Sync>);
        
        Self { handlers }
    }
    
    pub fn handle(&self, message: &proto::HsesRequestMessage, state: &mut MockState) -> Result<Vec<u8>, proto::ProtocolError> {
