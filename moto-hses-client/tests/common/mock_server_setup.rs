...
use log::error;
use moto_hses_mock::{
    MockConfig, MockServer, TypedVariables, VariableType, server::MockServerBuilder,
};
...
-> Result<MockServerManager, Box<dyn std::error::Error + Send + Sync>> {
    ...
    manager
        .start_with_builder(|builder| {
            ...
            variables.insert((VariableType::Integer, 0), vec![100, 0]); // I000 = 100
            variables.insert((VariableType::Integer, 1), vec![200, 0]); // I001 = 200

            // I32 variables (D variables) - indices 10, 11
            variables.insert((VariableType::Double, 10), vec![232, 3, 0, 0]); // D010 = 1000
            variables.insert((VariableType::Double, 11), vec![208, 7, 0, 0]); // D011 = 2000

            // F32 variables (R variables) - indices 20, 21
            variables.insert((VariableType::Real, 20), vec![0, 0, 192, 63]); // R020 = 1.5
            variables.insert((VariableType::Real, 21), vec![0, 0, 32, 64]); // R021 = 2.5

            // U8 variables (B variables) - indices 30, 31
            variables.insert((VariableType::Byte, 30), vec![10]); // B030 = 10
            variables.insert((VariableType::Byte, 31), vec![20]); // B031 = 20

            // String variables (S variables) - indices 40, 41
            variables.insert((VariableType::String, 40), b"Hello".to_vec()); // S040 = "Hello"
            variables.insert((VariableType::String, 41), b"World".to_vec()); // S041 = "World"
